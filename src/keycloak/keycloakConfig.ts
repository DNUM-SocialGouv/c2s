const isDateLessThanTwentyFourHours = (storedDate: string | number | Date | null): boolean => {
    if (storedDate) {
...
function allValuesExist(obj: { [key: string]: any }): boolean {
...
export const fetchKeycloakConfig = async () => {
    try {
        ...
        if(localConfigs && allValuesExist(localConfigs) && externlApps && allValuesExist(externlApps) && isDateLessThanTwentyFourHours(storedDate!)){
